<template>
  <div class="app-container">
    <h3 class="ranking">角色权限</h3>
    <div class="filter-container">
      <el-select clearable class="filter-item" style="width: 130px" v-model="listQuery.type" :placeholder="$t('goods.type')">
      ...
      <el-button class="filter-item" style="margin-left: 10px;" @click="handleCreate" type="primary" icon="el-icon-edit">{{$t('goods.add')}}</el-button>
    </div>
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row style="width: 100%;min-height:1000px;">
      ...
      <el-table-column align="center" label="姓名" min-width="100"> <!-- 机器人编号 -->
        <template slot-scope="scope">
          <span>{{scope.row.machineId}}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="100px" label="微信ID" align="center"> <!-- 机器人名称以及楼层 -->
        <template slot-scope="scope">
      ...
      <el-table-column width="200px" label="角色身份" align="center"> <!-- 机器人名称以及楼层 -->
        <template slot-scope="scope">
          <span class="link-type" @click="handleUpdate(scope.row, scope.$index)">{{scope.row.name}}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="80px" align="center" label="添加时间"> <!-- 商品保质期 -->
        <template slot-scope="scope">
          <span>{{scope.row.deadDate | parseTime('{y}-{m}-{d}')}}</span> <!-- 使用过滤器，商品保质期只显示年-月-日 -->
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" style="margin-top: -6%"> <!-- "添加"和"编辑"按钮弹出的界面模板 -->
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="100px" style='width: 450px; margin-left:50px;'>
        <el-form-item label="姓名" prop="type"> <!-- 选择新商品类型 -->
          <el-input v-model="input" placeholder="请输入内容"></el-input>
        </el-form-item>
        <el-form-item label="微信ID" prop="vendorId"> <!-- 选择商品的供应商名字 -->
          <el-input v-model="input" placeholder="请输入内容"></el-input>
        </el-form-item>
        <el-form-item label="联系电话" prop="vendorId"> <!-- 选择商品的供应商名字 -->
          <el-input v-model="input" placeholder="请输入内容"></el-input>
        </el-form-item>
        <el-form-item label="角色身份" prop="vendorId">
