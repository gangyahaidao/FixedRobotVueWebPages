<template>
  <div class="app-container">
    <h3 class="ranking">机器人列表</h3>
    <div class="filter-container">
      <span class="type filter-item" >机器人名称</span>
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row style="width: 100%;min-height:1000px;">
      ...
      <el-table-column align="center" label="机器人编码" min-width="100"> <!-- 机器人编号 -->
        <template slot-scope="scope">
      ...
      <el-table-column width="200px" label="货柜楼层" align="center"> <!-- 机器人名称以及楼层 -->
        <template slot-scope="scope">
          <span class="link-type" @click="handleUpdate(scope.row, scope.$index)">{{scope.row.name}}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="80px" align="center" label="添加时间"> <!-- 商品保质期 -->
        <template slot-scope="scope">
          <span>{{scope.row.deadDate | parseDateObj}}</span> <!-- 使用过滤器，商品保质期只显示年-月-日 -->
        </template>
      </el-table-column>
