<template>
  <div :id="id" :style="{height:height,width:width}" v-loading="listLoading"></div>
</template>
...
export default {
  ...
  methods: {
    refreshChartData(val) { // 父页面点击不同机器人重新加载不同的数据
      ...
      alert(this.robotMachineId)
      this.getCharDataByFloorAndMachineId()
    },
    getCharDataByFloorAndMachineId() { // 根据楼层和机器人编号获取售卖商品信息
      getRobotSaleData(this.floorName, this.robotMachineId, 2).then(response => {
